...
export async function PUT(request: Request) {
  try {
    ...
    const cv = cvSchema.parse(body.cv);
    const version = await saveDraft(cv, Number(body.lockVersion));
    return Response.json({ lockVersion: version });
  } catch (e) {
    if (e instanceof Error && e.message === "STALE_DRAFT")
      return Response.json(
        { error: "Draft changed elsewhere. Reload before saving." },
        { status: 409 },
      );
...
export async function POST(request: Request) {
  try {
    await requireAdmin();
    const body = await request.json();
    if (body.action === "publish") await publishDraft();
    else if (body.action === "restore" && typeof body.id === "string")
      await restoreRevision(body.id);
